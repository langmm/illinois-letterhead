% https://tex.stackexchange.com/questions/59932/reproducing-an-official-letterhead

% Colors
% blue:   CMYK 100/90/10/77
% orange: CMYK 01/83/85/0
%\usepackage[cmyk]{xcolor}
%\definecolor{UIblue}{cmyk}{1.00, 0.90, 0.10, 0.77}
%\definecolor{UIorange}{cmyk}{0.01, 0.83, 0.85, 0.00}
\usepackage[rgb]{xcolor}
\definecolor{UIblue}{RGB}{19,41,75}
\definecolor{UIorange}{RGB}{232,74,39}

% set page geometry
\usepackage[top=1in,left=1in,bottom=1in,right=1.0in]{geometry}
\usepackage{graphicx}

% logo
\providecommand\logofile{Illinois-Wordmark-Horizontal-Full-Color-CMYK.pdf}

% remove coloring from URLs
\usepackage[colorlinks=false,pdfborder={0 0 0}]{hyperref}
\urlstyle{same}

% to place things in exact places
%\usepackage[absolute,showboxes]{textpos}
\usepackage[absolute]{textpos}

% for conditionals
\usepackage{ifthen}

% load Montserrat font -- this is a close alternative to Gotham
\usepackage[defaultfam,light,tabular,lining]{montserrat} %% Option 'defaultfam'
%% only if the base font of the document is to be sans serif
\usepackage[T1]{fontenc}

% set linespacing and paragraph spacing
\linespread{1.10}
\setlength{\parskip}{1em}

% use microtype to extend spacing
\usepackage{microtype}

% Remove paragraph indentation
\parindent0pt
\raggedright%

% change page style to include header/footer
\pagestyle{empty}

% add stuff to the doc
\AtBeginDocument{%
    % Top block with logo
    \begin{textblock*}{\textwidth}(1in,0.75in)
      \includegraphics[height=0.3125in]{\logofile}
    \end{textblock*}

    % Inset department address
    %\begin{textblock*}{4.0in} (1in,1.65625in)
    \begin{textblock*}{4.0in} (1in,1.40625in)
        {%
         % use medium bold here
        \color{UIblue}
        \fontsize{10}{12}\selectfont
        \fontseries{mb}\selectfont
        \fromdept\\%
        }
        \vspace{2mm}
        {%
        \color{UIblue}
        \footnotesize\par
        \fontsize{9}{12}\selectfont
        %\fontsize{9}{10}\selectfont
        \fromdeptaddress%
        }
    \end{textblock*}

    % main text
    %\newgeometry{top=3.0in,left=0.75in,bottom=1.2in,right=1.0in}
    \vspace*{-1in}
  }

\usepackage{fancyhdr}
\pagestyle{fancy}

\renewcommand{\footrulewidth}{0pt}
\fancyfoot{}
\fancyfoot[L]{%
    % footer
    % specify the top right corner location of the footer
    \begin{textblock*}{\textwidth}[1.0,0.0](7.5in,9.9375in)
        {%
         \begin{flushright}
         \color{UIorange}
         \fontseries{mb}\selectfont
         UNIVERSITY OF ILLINOIS AT URBANA-CHAMPAIGN
         \end{flushright}
        }
        \vspace{-5mm}
        {%
         \begin{flushright}
         \color{UIblue}
         \fontsize{9}{11}\selectfont
         \fromtel\ \textbullet\
         \fromemail\ \textbullet\
         \fromweb%
         \end{flushright}
        }
    \end{textblock*}
    }

\fancyhead{}
\renewcommand{\headrulewidth}{0pt}

% add closing to end of the document, if sig is available
\AtEndDocument{%
    \par\vspace{2ex}
    \closing,

    \ifthenelse{\equal{\signaturefile}{}}{%
    	\bigskip%\bigskip%
    }{\includegraphics[width=1.2in]{\signaturefile}\\[-0.2\baselineskip]}

    \fromname%
}
